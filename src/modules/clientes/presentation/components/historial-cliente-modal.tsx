...
/**
 * Determina si un cambio de campo corresponde a una baja lógica
...
export const HistorialClienteModal = ({
  cliente,
}: HistorialClienteModalProps) => {
  const logs = parseCambiosLog(cliente.cambios_log);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 bg-zinc-900/20 rounded-2xl border border-dashed border-zinc-800">
        ...
        <p className="text-zinc-500 text-sm font-medium">
          No se encontraron cambios registrados para este cliente.
        </p>
      </div>
    );
  }
      ...
      >
        <AnimatePresence>
          {logsOrdenados.map((log, idx) => {
            ...
            const titulo = esBaja ? "Cliente desactivado" : "Edición";
